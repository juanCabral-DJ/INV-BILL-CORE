...
def estado_inventario(umbral_bajo: int = Query(5, ge=0), db: Session = Depends(get_db)):
    return reportes_service.estado_inventario(db=db, umbral_bajo=umbral_bajo)


@router.get("/inventario/excel")
def inventario_excel(umbral_bajo: int = Query(5, ge=0), db: Session = Depends(get_db)):
